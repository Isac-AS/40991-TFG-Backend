...
from src import db, ResponseCreator
...
def create_health_record_from_audio():
    ...
    :param audio_file: Audio to be saved
    :type audio_file: blob
    :return: Standard response
    :rtype: http_response
    """
    audio_file = request.get_data("audio")
    pipeline_id = request.json.get("pipeline_id")

    #pipeline_output = run_pipeline(pipeline_id, 0, strategy_input)
    pass
...
def save_audio():
    ...
    return ResponseCreator.create_response(result=True, message="Audio guardado correctamente", caller="Path", object=audio_file_path)
...
def run_pipeline(pipeline_id, skip_steps, strategy_input):
    ...
    for strategy in strategies_to_run:
        strategy_output = run_strategy(strategy_input=strategy_input, strategy_id=strategy["id"])
...
def run_strategy(strategy_input, strategy_id):
    ...
    strategy: Strategy = db.session.execute(db.select(Strategy).filter_by(id = strategy_id )).scalar_one()
    # Run strategy as subprocess
    process = subprocess.Popen([strategy.env_path, strategy.python_file_path, strategy_input], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
